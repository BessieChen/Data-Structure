...
//这是第五版，旨在让findID()能快一点。方法：将路径压缩到最短
public class UnionFind6 implements UF //注意，里面的装的，不是Element！
{
    ...
    //method 2:递归
    private int findID_R(int i)
    {
        if(i < 0 || i >= parent.length)
            throw new IllegalArgumentException("p is out of bound.");

        //错误的递归：//todo 解释 + 复制到547
//   wrong：if(i != parent[i])
//                parent[i] = findID_R(i);

        //正确：
        if(i != parent[i])
            parent[i] = findID_R(parent[i]);
        return parent[i];
    }
