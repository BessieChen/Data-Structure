public class Array<Element> {
     ...
     * 6. 用到Array的其他数据结构：
     * -- MaxHeap
     * 7. 设置泛型的时候，我们的remove()除了有size--, 还需要有data[size] = null, 这是因为如果是泛型，代表我们data里面包含的是对象，对象只有在没有指针指向她的时候，才被java垃圾回收处理，这里无法垃圾回收，我们需要自己设置回收。
     *    注意这个是loitering object就是没有指针认领的object，但是这个不等同于memory leak（浪费内存）
     *
     * 8.易错：
